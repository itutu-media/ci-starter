...
class IM_MenuBuilder
{
    ...
    foreach ($array as $element) {
      if ($element['parent_id'] == $parent_id) {
        if (in_array($element['id'], $parents)) {
          if (strpos($path, $element['url']) !== false)
  ...
  public function userSidebarBuilder($array, $parent_id = 0, $parents = [])
  {
    if ($parent_id == 0) {
      foreach ($array as $element) {
        if (($element['parent_id'] != 0) && !in_array($element['parent_id'], $parents)) {
    ...
    foreach ($array as $element) {
      if ($element['parent_id'] == $parent_id) {
        $menu_html .= '<a href="' . $element['url'] . '" class="' . $element['styling'] . '">' . $element['title'] . '</a>';
      }
    }
